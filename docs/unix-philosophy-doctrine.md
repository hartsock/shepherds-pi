Reference doctrine for the [unix-philosophy shepherd skill](../skills/unix-philosophy/SKILL.md).
...
- **Do one thing well.** A program that tries to do everything does
  ...
  to learn to ignore. Unrelated responsibilities are the signal to reconsider the boundary;
  a long help page alone is not.
...
A well-behaved Unix program emits its result data without reassurance.
A command with no result data may be silent on success. Chatty success output is noise
